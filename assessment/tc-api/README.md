...
Derived from controllers under `api/app/controllers/api/v1/`, models under `api/app/models/`, middleware, and routes (`api/config/routes.rb`). Format follows `assessment/test-api-v1/`.

## Suites

| File | Coverage |
|------|----------|
| [01-auth-and-health.md](01-auth-and-health.md) | Health, speed test, login, auth/tenant failures |
| [02-assessments.md](02-assessments.md) | Assessment CRUD + validation |
...
| [05-vacancies.md](05-vacancies.md) | Vacancy CRUD + validation |
| [06-skill-taxonomies.md](06-skill-taxonomies.md) | Skill taxonomy read APIs |
| [e2e-business-flow.md](e2e-business-flow.md) | Chained create → list → taxonomies → show → skill add/remove (process) |
...
4. Redis available (Rack::Attack login throttle uses Redis).
5. Sidekiq running for portfolio / fit-gap generation workers (suite 04).
...
2. **Message** matches **Expected message** when present (`errors[0].message` for errors, or top-level `message` / `error`)
3. **Payload** contains the keys/types listed under **Expected payload (validate)**

Error envelope shape (most controller errors):
...
Rate-limit (Rack::Attack) uses a different shape:
...
2. `e2e-business-flow` (optional process smoke; needs `$TOKEN` only) **or** `02-assessments` → capture `$ASSESSMENT_ID`
