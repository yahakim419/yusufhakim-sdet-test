...
Evidence: `api/app/controllers/api/v1/sessions_controller.rb`, `api/app/models/session.rb`, `api/app/services/sessions/end_handler.rb`.

`END_REASONS`: `manual_candidate`, `manual_assessor`, `all_covered`, `time_ceiling`, `error`.
...
- **Expected payload (validate):**
  ...
  - `session.status` is typically `"pending"` on create
  ```json
  {
    "session": {
      "id": 1,
...
### TC-SES-002 — Create session for missing assessment
- **Priority:** P1
- **Method / Path:** `POST /api/v1/assessments/:assessment_id/sessions`
- **Auth:** Bearer JWT
- **curl:**
  ```bash
  curl -i -X POST "$BASE/api/v1/assessments/999999/sessions" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{
      "session": {
...
### TC-SES-003 — List sessions for assessment
- **Priority:** P0
...
### TC-SES-004 — List sessions for missing assessment
- **Priority:** P1
- **Method / Path:** `GET /api/v1/assessments/:assessment_id/sessions`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-SES-006 — Get session not found
- **Priority:** P1
- **Method / Path:** `GET /api/v1/sessions/:id`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-SES-009 — Candidate info (public invite token)
- **Priority:** P0
...
### TC-SES-010 — Candidate info invalid token
- **Priority:** P1
- **Method / Path:** `GET /api/v1/sessions/:token/candidate`
- **Auth:** invite token
- **curl:**
...
### TC-SES-011 — End session (assessor)
- **Priority:** P0
...
- **Expected payload (validate):**
  ...
  ```
- **Expected state change:** Portfolio record created with `generation_status: pending` (Sidekiq worker enqueued)

---

### TC-SES-012 — End session invalid reason
- **Priority:** P1
- **Method / Path:** `POST /api/v1/sessions/:id/end_session`
- **Auth:** Bearer JWT
...
### TC-SES-013 — End session already ended
- **Priority:** P1
- **Method / Path:** `POST /api/v1/sessions/:id/end_session`
- **Auth:** Bearer JWT
- **Precondition:** Session from TC-SES-011 already ended
- **curl:**
  ```bash
  curl -i -X POST "$BASE/api/v1/sessions/$SESSION_ID/end_session" \
    -H "Authorization: Bearer $TOKEN" \
...
### TC-SES-014 — End session not found
- **Priority:** P2
- **Method / Path:** `POST /api/v1/sessions/:id/end_session`
- **Auth:** Bearer JWT
- **curl:**
  ```bash
  curl -i -X POST "$BASE/api/v1/sessions/999999/end_session" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{
      "session": { "reason": "manual_assessor" }
...
### TC-SES-015 — Audio complete with invalid token
- **Priority:** P1
...
### TC-SES-016 — Audio complete when session already ended (idempotent)
- **Priority:** P1
- **Method / Path:** `POST /api/v1/sessions/:token/audio_complete`
- **Auth:** invite token
...
### TC-SES-017 — Audio complete ends active session
- **Priority:** P1
- **Method / Path:** `POST /api/v1/sessions/:token/audio_complete`
- **Auth:** invite token
- **Precondition:** Fresh active/pending session (not ended). Endpoint calls `EndHandler` with reason `all_covered`.
- **curl:**
  ```bash
...
### TC-SES-018 — Create session rejects missing token
- **Priority:** P0
- **Method / Path:** `POST /api/v1/assessments/:assessment_id/sessions`
- **Auth:** none
- **curl:**
  ...
  curl -i -X POST "$BASE/api/v1/assessments/$ASSESSMENT_ID/sessions" \
    ...
    -d '{"session":{"candidate_name":"No Auth"}}'
  ```
- **Expected status:** `401`
