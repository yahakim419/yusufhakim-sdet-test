...
Evidence: `api/app/controllers/api/v1/portfolios_controller.rb`, `api/app/controllers/api/v1/portfolio_skills_controller.rb`, `api/app/models/portfolio.rb`, `api/app/models/assessor_override.rb`.
...
- **Auth:** Bearer JWT
- **Precondition:** Portfolio missing or `generation_status` is `generating`
- **curl:**
  ```bash
...
### TC-PF-004 — Get portfolio session not found
- **Priority:** P2
- **Method / Path:** `GET /api/v1/sessions/:id/portfolio`
- **Auth:** Bearer JWT
- **curl:**
  ```bash
  curl -i "$BASE/api/v1/sessions/999999/portfolio" \
    -H "Authorization: Bearer $TOKEN"
  ```
- **Expected status:** `404`
...
### TC-PF-005 — Regenerate portfolio when failed
- **Priority:** P1
...
### TC-PF-006 — Regenerate portfolio rejected when not failed
- **Priority:** P1
- **Method / Path:** `POST /api/v1/sessions/:id/portfolio/regenerate`
- **Auth:** Bearer JWT
...
### TC-PF-007 — Regenerate when no portfolio
- **Priority:** P2
- **Method / Path:** `POST /api/v1/sessions/:id/portfolio/regenerate`
- **Auth:** Bearer JWT
...
### TC-PF-008 — Fit/gap missing vacancy_id
- **Priority:** P1
...
### TC-PF-009 — Fit/gap vacancy not found
- **Priority:** P1
- **Method / Path:** `POST /api/v1/portfolios/:id/fitgap`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-PF-010 — Fit/gap rejected when portfolio not ready
- **Priority:** P1
- **Method / Path:** `POST /api/v1/portfolios/:id/fitgap`
- **Auth:** Bearer JWT
- **Precondition:** Portfolio exists but `generation_status` is not complete (e.g. `pending` / `generating` / `failed`)
- **curl:**
  ```bash
  curl -i -X POST "$BASE/api/v1/portfolios/$NOT_READY_PORTFOLIO_ID/fitgap" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d "{\"vacancy_id\":$VACANCY_ID}"
  ```
- **Expected status:** `422`
- **Expected message:** `Portfolio is not ready (status: <generation_status>)`
- **Expected payload (validate):**
  - `errors[0].status` == `422`
  - `errors[0].message` starts with `"Portfolio is not ready (status:"`
  ```json
  {
    "errors": [
      { "status": 422, "message": "Portfolio is not ready (status: pending)" }
    ]
  }
  ```
...
### TC-PF-011 — Fit/gap queue generation (no cache)
- **Priority:** P0
- **Method / Path:** `POST /api/v1/portfolios/:id/fitgap`
- **Auth:** Bearer JWT
...
### TC-PF-012 — Fit/gap returns cached report
- **Priority:** P0
- **Method / Path:** `POST /api/v1/portfolios/:id/fitgap`
- **Auth:** Bearer JWT
...
- **Expected message:** `n/a`
- **Expected payload (validate):**
  - Same `report` shape as TC-PF-012
  ```json
  {
    "report": {
      "id": 1,
...
### TC-PF-014 — Get fit/gap report not found
- **Priority:** P1
- **Method / Path:** `GET /api/v1/portfolios/:id/fitgap/:vacancy_id`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-PF-016 — Regenerate fit/gap missing vacancy_id
- **Priority:** P1
- **Method / Path:** `POST /api/v1/portfolios/:id/regenerate_fitgap`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-PF-017 — Export portfolio JSON
- **Priority:** P0
...
### TC-PF-018 — Export invalid format
- **Priority:** P1
- **Method / Path:** `GET /api/v1/portfolios/:id/export`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-PF-019 — Export rejected when portfolio not ready
- **Priority:** P1
- **Method / Path:** `GET /api/v1/portfolios/:id/export`
- **Auth:** Bearer JWT
- **Precondition:** Portfolio exists but `generation_status` is not `complete`
- **curl:**
  ```bash
  curl -i "$BASE/api/v1/portfolios/$NOT_READY_PORTFOLIO_ID/export?format=json" \
    -H "Authorization: Bearer $TOKEN"
  ```
- **Expected status:** `422`
- **Expected message:** `Portfolio is not ready for export (status: <generation_status>)`
- **Expected payload (validate):**
  - `errors[0].message` starts with `"Portfolio is not ready for export (status:"`
  ```json
  {
    "errors": [
      {
        "status": 422,
        "message": "Portfolio is not ready for export (status: pending)"
      }
    ]
  }
  ```
...
### TC-PF-020 — Override portfolio skill
- **Priority:** P0
...
### TC-PF-021 — Override rejects invalid override_level
- **Priority:** P1
- **Method / Path:** `POST /api/v1/portfolio_skills/:id/override`
- **Auth:** Bearer JWT
- **Precondition:** `$PORTFOLIO_SKILL_ID` exists
- **curl:**
  ```bash
  curl -i -X POST "$BASE/api/v1/portfolio_skills/$PORTFOLIO_SKILL_ID/override" \
    -H "Authorization: Bearer $TOKEN" \
    ...
    -d '{
      "override": {
        "override_level": 9,
        "assessor_notes": "out of range"
      }
    }'
  ```
- **Expected status:** `422`
- **Expected message:** (numericality; assert non-empty `errors[0].message` mentioning override level)
- **Expected payload (validate):**
  - `errors[0].status` == `422`
  ```json
  {
    "errors": [
      { "status": 422, "message": "Override level must be in 1..5" }
    ]
  }
  ```
...
### TC-PF-022 — Override portfolio skill not found
- **Priority:** P1
- **Method / Path:** `POST /api/v1/portfolio_skills/:id/override`
- **Auth:** Bearer JWT
- **curl:**
...
### TC-PF-023 — Portfolio not found on fitgap
- **Priority:** P2
- **Method / Path:** `POST /api/v1/portfolios/:id/fitgap`
- **Auth:** Bearer JWT
- **curl:**
